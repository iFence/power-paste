...
mod tests {
    ...
    fn accepts_supported_debug_status() {
        ...
        let next = build_debug_status("0.3.1".into(), payload).expect("debug status");

        assert_eq!(next.status, "available");
        assert_eq!(next.current_version, "0.3.1");
    ...
    fn rejects_unsupported_debug_status() {
        ...
        let error = build_debug_status("0.3.1".into(), payload).expect_err("invalid status");

...
- Linux：已支持剪贴板监听、写回系统剪贴板、历史浏览、托盘、更新检查和全局快捷键；为保证不同桌面环境下行为一致，当前暂不支持直接粘贴、图文混合回放和开机启动
...
- Linux 上直接粘贴回上一个目标应用
- 开机启动
- Windows 之外的原生图文混合内容回放

Linux 上的历史浏览、剪贴板监听、搜索、筛选、置顶、编辑、删除、托盘、检查更新、设置保存等通用能力仍然可用。
